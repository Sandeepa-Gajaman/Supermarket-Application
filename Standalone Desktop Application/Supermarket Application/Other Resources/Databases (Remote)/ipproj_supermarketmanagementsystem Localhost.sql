...
-- Procedures
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `select_all_items`()
begin
select * from Item;
end$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `select_items_by_brand`(in brand varchar(25))
begin
select * from Item where item_brand like concat ('%', brand, '%');
end$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `select_items_by_category`(IN `category` VARCHAR(25))
begin
select * from Item where item_category 
like concat ('%', category, '%');
end$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `select_items_by_code`(IN `code` INT(7))
begin
select * from Item where item_code like concat ('%', code, '%');
end$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `select_items_by_name`(in name varchar(25))
begin
